...
class FSBW_Public_Display
{
    public function render_floating_buttons($options = null, $is_preview = false)
    {
        ...
        if ($show_on_all_pages || $current_page_id == $selected_page_id || $is_preview) {
...
?>
            <div id="fsbw-container" class="fsbw-container <?php echo esc_attr($position_class); ?>">
                <div id="fsbw-buttons" class="fsbw-buttons">
                    <?php
                    foreach ($buttons as $button) {
                        $button_color = isset($button['icon_color']) ? ltrim($button['icon_color'], '#') : 'ffffff';
                    ?>
                        <a href="<?php echo esc_url($button['link']); ?>" target="_blank" class="fsbw-button <?php echo esc_attr($style_class); ?> <?php echo esc_attr($size_class); ?>" style="background-color: <?php echo esc_attr($button['bg_color']); ?>; border-radius: <?php echo esc_attr($style_class == 'rounded' ? $border_radius : '50%'); ?>;">
                            <img src="https://cdn.simpleicons.org/<?php echo esc_attr($button['icon']); ?>/<?php echo esc_attr($button_color); ?>" width="<?php echo esc_attr($icon_size); ?>" height="<?php echo esc_attr($icon_size); ?>" />
                        </a>
                    <?php
                    }
                    ?>
                </div>
                <div class="fsbw-main-icon fsbw-button <?php echo esc_attr($style_class); ?> <?php echo esc_attr($size_class); ?>" style="background-color: <?php echo esc_attr($main_icon_bg_color); ?>; border-radius: <?php echo esc_attr($style_class == 'rounded' ? $border_radius : '50%'); ?>;">
                    <?php echo wp_kses(str_replace(array('fill="white"', 'width="32"', 'height="32"'), array('fill="#' . esc_attr($main_icon_color) . '"', 'width="' . esc_attr($icon_size) . '"', 'height="' . esc_attr($icon_size) . '"'), $main_icon_svg), $allowed_tags); ?>
                </div>
            </div>
            <style>
                <?php echo isset($options['fsbw_custom_css']) ? esc_attr('.fsbw-button {' . $options['fsbw_custom_css'] . '}') : ''; ?>
            </style>
<?php
        }
    }
    ...
    public function enqueue_styles_and_scripts()
    {
        ...
        $style_version = filemtime(plugin_dir_path(__FILE__) . 'styles.css');
        $script_version = filemtime(plugin_dir_path(__FILE__) . 'scripts.js');

        wp_enqueue_style('fsbw-styles', plugins_url('styles.css', __FILE__), array(), $style_version);
        wp_enqueue_script('fsbw-scripts', plugins_url('scripts.js', __FILE__), array('jquery'), $script_version, true);
    }
}
